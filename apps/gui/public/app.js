...
function renderFontSystemOptions(previous = []) {
  const systems = state.fonts || [];
  const selected = new Set(Array.isArray(previous) ? previous : previous ? [previous] : []);
  $("#build-font-systems").innerHTML = systems.map(system => `<label class="check"><input class="font-system-choice" type="checkbox" value="${escAttr(system.id)}" ${selected.has(system.id) ? "checked" : ""}><span>${esc(system.name)}</span><span class="meta">${system.faces.length} face${system.faces.length === 1 ? "" : "s"}</span></label>`).join("") || `<span class="muted">Import font profiles to select them here.</span>`;
  document.querySelectorAll("#build-font-systems .font-system-choice").forEach(input => input.onchange = syncFontSelectors);
}
...
function syncFontSelectors() {
  ...
  const choices = [...document.querySelectorAll("#build-font-systems .font-system-choice")];
  if (hasDesignSystem) choices.forEach(input => { input.checked = false; });
  choices.forEach(input => { input.disabled = hasDesignSystem; });
  $("#build-design-system").disabled = choices.some(input => input.checked);
}

function selectedFontSystemIds() {
  return [...document.querySelectorAll("#build-font-systems .font-system-choice:checked")].map(input => input.value);
}

function profileFontNames(profile) {
  const ids = Array.isArray(profile.fontSystems) && profile.fontSystems.length
    ? profile.fontSystems
    : profile.fontSystem ? [profile.fontSystem] : [];
  return ids.map(value => {
    const id = typeof value === "string" ? value : value.id;
    return state.fonts.find(system => system.id === id)?.name || id;
  }).join(", ");
}
...
function freshTypographyDraft() {
  ...
  return { roles, custom:{}, globalTypography:{primary:structuredClone(roles.h1),secondary:structuredClone(roles.h2),text:structuredClone(roles.body),accent:structuredClone(roles.links)}, globalCustomTypography:[], fallbackFontFamily:"sans-serif", fontSlots:{ body:{name:"Body font"}, heading:{name:"Heading font"}, accent:{name:"Accent font"} }, customRoleNames:{} };
}
function restoreTypeDraft() { if (!typographyDraft) { try { typographyDraft = JSON.parse(sessionStorage.getItem("wpstarter.typographyDraft")) || freshTypographyDraft(); } catch { typographyDraft = freshTypographyDraft(); } } if(!typographyDraft.fontSlots) typographyDraft.fontSlots={body:{name:"Body font"},heading:{name:"Heading font"},accent:{name:"Accent font"}}; typographyDraft.custom ||= {}; typographyDraft.customRoleNames ||= {}; for(const [id] of TYPE_ROLES) if(!typographyDraft.roles?.[id]) typographyDraft.roles[id]=freshTypographyDraft().roles[id]; typographyDraft.globalTypography ||= {}; const legacyGlobals={primary:typographyDraft.roles.h1||typographyDraft.roles.body,secondary:typographyDraft.roles.h2||typographyDraft.roles.body,text:typographyDraft.roles.body,accent:typographyDraft.roles.links||typographyDraft.roles.body}; for(const [id,token] of Object.entries(legacyGlobals))if(!typographyDraft.globalTypography[id])typographyDraft.globalTypography[id]=structuredClone(token); typographyDraft.globalCustomTypography ||= []; typographyDraft.fallbackFontFamily ||= "sans-serif"; }
function saveTypeDraft() { sessionStorage.setItem("wpstarter.typographyDraft", JSON.stringify(typographyDraft)); }
function dimensionControl(roleId, property, units, token, scope="type") {
  const current = token[property]?.[activeTypeDevice]; const unit = current?.unit ?? (property === "lineHeight" ? "" : "px");
  return `<div class="dimension-control"><input type="number" step="any" data-scope="${scope}" data-role="${escAttr(roleId)}" data-dimension="${property}" value="${current?.value ?? ""}" placeholder="—"><select data-scope="${scope}" data-role="${escAttr(roleId)}" data-unit="${property}">${units.map(value => `<option value="${escAttr(value)}" ${value===unit?"selected":""}>${value || "unitless"}</option>`).join("")}</select></div>`;
}
function typographyTokenFor(scope,id){if(scope==="global"){return typographyDraft.globalTypography?.[id]||typographyDraft.globalCustomTypography?.find(item=>item.id===id)?.token;}return typographyDraft.roles[id]||typographyDraft.custom[id];}
function renderTypographyDraft() {
  ...
  $("#type-rows").innerHTML = rows.map(([id,label,token,custom]) => `<tr data-type-row="${escAttr(id)}"><td>${custom?`<input class="role-name" data-role="${escAttr(id)}" data-role-name="${escAttr(id)}" value="${escAttr(label)}">`:`<strong>${esc(label)}</strong>`}</td><td><select data-scope="type" data-role="${escAttr(id)}" data-shared="fontRole">${slotOptions}<option value="__add">+ Add slot…</option></select></td><td><select data-scope="type" data-role="${escAttr(id)}" data-shared="weight">${[100,200,300,400,500,600,700,800,900].map(w=>`<option ${Number(token.weight)===w?"selected":""}>${w}</option>`).join("")}</select></td><td><select data-scope="type" data-role="${escAttr(id)}" data-shared="style">${["normal","italic","oblique"].map(v=>`<option ${token.style===v?"selected":""}>${v}</option>`).join("")}</select></td>${TYPE_DIMENSIONS.map(([property,,units])=>`<td>${dimensionControl(id,property,units,token)}</td>`).join("")}<td><select data-scope="type" data-role="${escAttr(id)}" data-shared="textTransform">${["none","uppercase","lowercase","capitalize"].map(v=>`<option ${token.textTransform===v?"selected":""}>${v}</option>`).join("")}</select></td><td><select data-scope="type" data-role="${escAttr(id)}" data-shared="textDecoration">${["none","underline","overline","line-through"].map(v=>`<option ${token.textDecoration===v?"selected":""}>${v}</option>`).join("")}</select></td><td>${custom?`<button class="tiny danger remove-custom-type" data-role="${escAttr(id)}">Remove</button>`:""}</td></tr>`).join("");
  ...
  document.querySelectorAll("#type-rows input,#type-rows select").forEach(input => input.oninput = () => updateTypographyDraftField(input));
  ...
  renderGlobalFontsDraft();
}
function updateTypographyDraftField(input){const id=input.dataset.role;if(!id)return;const scope=input.dataset.scope||"type",token=typographyTokenFor(scope,id);if(!token)return;if(input.dataset.shared){if(input.dataset.shared==="fontRole"&&input.value==="__add"){const label=prompt("Name this font slot (for example, Display font):");if(!label)return renderTypographyDraft();const slotId=`slot-${Date.now().toString(36)}`;typographyDraft.fontSlots[slotId]={name:label.trim()};token.fontRole=slotId;renderTypographyDraft();return;}token[input.dataset.shared]=input.dataset.shared==="weight"?Number(input.value):input.value;}if(input.dataset.dimension){token[input.dataset.dimension]||={};const unit=document.querySelector(`[data-scope="${scope}"][data-role="${CSS.escape(id)}"][data-unit="${input.dataset.dimension}"]`).value;if(input.value==="")delete token[input.dataset.dimension][activeTypeDevice];else token[input.dataset.dimension][activeTypeDevice]={value:Number(input.value),unit};}if(input.dataset.unit){const number=document.querySelector(`[data-scope="${scope}"][data-role="${CSS.escape(id)}"][data-dimension="${input.dataset.unit}"]`);if(number.value!==""){token[input.dataset.unit]||={};token[input.dataset.unit][activeTypeDevice]={value:Number(number.value),unit:input.value};}}if(input.dataset.roleName)typographyDraft.customRoleNames[id]=input.value;saveTypeDraft();}
function renderGlobalFontsDraft(){restoreTypeDraft();const slots=Object.entries(typographyDraft.fontSlots||{}).map(([id,slot])=>`<option value="${escAttr(id)}">${esc(slot.name)}</option>`).join("");const aliases=[["primary","Primary"],["secondary","Secondary"],["text","Text"],["accent","Accent"]];const rows=[...aliases.map(([id,label])=>({id,label,token:typographyDraft.globalTypography[id],custom:false})),...(typographyDraft.globalCustomTypography||[]).map(item=>({id:item.id,label:item.name,token:item.token,custom:true}))];$("#global-font-rows").innerHTML=rows.map(({id,label,token,custom})=>`<tr><td>${custom?`<input data-global-name="${escAttr(id)}" value="${escAttr(label)}" aria-label="Custom Global Font name">`:`<strong>${label} Global Font</strong>`}</td><td><select data-scope="global" data-role="${escAttr(id)}" data-shared="fontRole">${slots}<option value="__add">+ Add slot…</option></select></td><td><select data-scope="global" data-role="${escAttr(id)}" data-shared="weight">${[100,200,300,400,500,600,700,800,900].map(w=>`<option ${Number(token.weight)===w?"selected":""}>${w}</option>`).join("")}</select></td><td><select data-scope="global" data-role="${escAttr(id)}" data-shared="style">${["normal","italic","oblique"].map(v=>`<option ${token.style===v?"selected":""}>${v}</option>`).join("")}</select></td>${TYPE_DIMENSIONS.map(([property,,units])=>`<td>${dimensionControl(id,property,units,token,"global")}</td>`).join("")}<td><select data-scope="global" data-role="${escAttr(id)}" data-shared="textTransform">${["none","uppercase","lowercase","capitalize"].map(v=>`<option ${token.textTransform===v?"selected":""}>${v}</option>`).join("")}</select></td><td><select data-scope="global" data-role="${escAttr(id)}" data-shared="textDecoration">${["none","underline","overline","line-through"].map(v=>`<option ${token.textDecoration===v?"selected":""}>${v}</option>`).join("")}</select></td><td>${custom?`<button class="tiny danger remove-global-font" data-id="${escAttr(id)}">Remove</button>`:""}</td></tr>`).join("");for(const row of rows){const select=document.querySelector(`#global-font-rows [data-role="${CSS.escape(row.id)}"][data-shared="fontRole"]`);if(select)select.value=row.token.fontRole;}document.querySelectorAll("#global-font-rows input,#global-font-rows select").forEach(input=>input.oninput=()=>{if(input.dataset.globalName){const item=typographyDraft.globalCustomTypography.find(x=>x.id===input.dataset.globalName);if(item)item.name=input.value;}else updateTypographyDraftField(input);saveTypeDraft();});document.querySelectorAll(".remove-global-font").forEach(button=>button.onclick=()=>{typographyDraft.globalCustomTypography=typographyDraft.globalCustomTypography.filter(item=>item.id!==button.dataset.id);renderGlobalFontsDraft();saveTypeDraft();});$("#global-font-fallback").value=typographyDraft.fallbackFontFamily||"sans-serif";$("#global-font-fallback").oninput=event=>{typographyDraft.fallbackFontFamily=event.target.value;saveTypeDraft();};}
...
function render() {
  ...
  const previousFontSystems = [...document.querySelectorAll("#build-font-systems .font-system-choice:checked")].map(input => input.value);
...
async function createProfile() {
  ...
  const body = {
    ...
    pluginVersions,
    fontSystemIds: selectedFontSystemIds(),
...
async function populateProfileEditor(profile) {
  ...
  const savedFontSystemIds = Array.isArray(profile.fontSystems) ? profile.fontSystems.map(font => font.id) : profile.fontSystem?.id ? [profile.fontSystem.id] : [];
  document.querySelectorAll("#build-font-systems .font-system-choice").forEach(input => { input.checked = savedFontSystemIds.includes(input.value); });
...
function resetProfileEditor() {
  ...
  $("#build-config").value = "";
  document.querySelectorAll("#build-font-systems .font-system-choice").forEach(input => { input.checked = false; });
...
$("#type-save").onclick = async () => {restoreTypeDraft();const saved=await saveResource("typography", { schemaVersion: 1, id: $("#type-id").value.trim(), name: $("#type-name").value.trim(), roles: typographyDraft.roles, custom: typographyDraft.custom, fontSlots: typographyDraft.fontSlots, customRoleNames: typographyDraft.customRoleNames, globalTypography: typographyDraft.globalTypography, globalCustomTypography: typographyDraft.globalCustomTypography, fallbackFontFamily: typographyDraft.fallbackFontFamily });if(saved){$("#type-id").value=saved.id;sessionStorage.removeItem("wpstarter.typographyDraft");}};
$("#type-reset").onclick = resetTypographyEditor;
$("#global-font-add-custom").onclick = () => {restoreTypeDraft();const name=prompt("Name this Elementor Global Font (for example, Eyebrow):");if(!name?.trim())return;const id=`global-${Date.now().toString(36)}`;typographyDraft.globalCustomTypography.push({id,name:name.trim(),token:structuredClone(typographyDraft.globalTypography.text)});saveTypeDraft();renderGlobalFontsDraft();};
