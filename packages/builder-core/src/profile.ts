...
import type { BuildProfile, ProfileDocumentV3, ProfileDocumentV4, ProfileDocumentV5, ProfileDocumentV6 } from "./types.js";
...
async function loadLegacySchema1(raw: any, absolute: string): Promise<BuildProfile> {
  ...
  return {
    ...
    configExport: resolveLocal(raw.configExport), fontSystem: null,
...
async function loadRegistryProfile(raw: any, absolute: string, options: LoadProfileOptions, schemaVersion: 3 | 4 | 5 | 6): Promise<BuildProfile> {
  ...
  return {
    ...
    plugins, configExport: snapshot ? snapshot.absoluteZip : null, fontSystem,
    languageArchives: Array.isArray(raw.languageArchives) ? raw.languageArchives.map((archive: any, index: number) => ({ locale: requireString(archive.locale, `languageArchives[${index}].locale`), zip: resolveLocal(requireString(archive.zip, `languageArchives[${index}].zip`)) })) : []
  };
}
...
export async function loadProfile(profilePath: string, options: LoadProfileOptions = {}): Promise<BuildProfile> {
  ...
  else if ([3,4,5,6].includes(raw.schemaVersion)) profile = await loadRegistryProfile(raw, absolute, options, raw.schemaVersion);
  else throw new BuilderError("invalid_profile", "Only profile schemaVersion 1 through 6 are supported.");
...
export interface CreateProfileFromSnapshotOptions {
  libraryDir?: string; name?: string; locale?: string; excludePlugins?: string[]; wordpressVersion?: string; wordpressVariant?: string; themeVersion?: string; pluginVersions?: Record<string, string>; fontSystemId?: string | null;
}

export async function createProfileFromSnapshot(snapshotId: string, options: CreateProfileFromSnapshotOptions = {}): Promise<ProfileDocumentV6> {
  ...
  return { schemaVersion: 6, name: options.name?.trim() || snapshot.id, locale, wordpress: { version: wordpressVersion, variant: wordpressVariant }, theme: { slug: snapshot.theme.slug, version: themeVersion }, plugins, config: { id: snapshot.id }, fontSystem: options.fontSystemId ? { id: options.fontSystemId } : null, languageArchives: [] };
}

export interface CreateProfileFromPackagesOptions {
  libraryDir?: string; name: string; locale: string; wordpressVersion: string; wordpressVariant: string; themeSlug?: string | null; themeVersion?: string | null; plugins?: Record<string, string>; fontSystemId?: string | null;
}

export async function createProfileFromPackages(options: CreateProfileFromPackagesOptions): Promise<ProfileDocumentV6> {
  ...
  requireFontDependencies(plugins.map((plugin) => plugin.slug), options.fontSystemId);
  if (options.fontSystemId) await new FontSystemRegistry(libraryDir).resolve(options.fontSystemId);
  return { schemaVersion: 6, name, locale, wordpress: { version: wordpressVersion, variant: wordpressVariant }, theme, plugins, config: null, fontSystem: options.fontSystemId ? { id: options.fontSystemId } : null, languageArchives: [] };
}

export type { ProfileDocumentV3, ProfileDocumentV4, ProfileDocumentV5, ProfileDocumentV6 };
